...
scan_services() {
    full_subdomain=$1
    root_domain=$(echo "$full_subdomain" | awk -F. '{print $(NF-1)"."$NF}')  
    output_dir=~/recon/$root_domain/scan
    safe_name=$(echo "$full_subdomain" | sed 's/\./_/g')
    output_file="$output_dir/naabu_${safe_name}.txt"
...
dns_records() {
    subdomain=$1
    root_domain=$(echo "$subdomain" | awk -F. '{print $(NF-1)"."$NF}')
    output_dir=~/recon/$root_domain/dns-records
    safe_name=$(echo "$full_subdomain" | sed 's/\./_/g')
    output_file="$output_dir/dns-records_${safe_name}.txt"
    mkdir -p "$output_dir"

    output_file="$output_dir/dns_$(echo $subdomain | cut -d'.' -f1).txt"

    echo "[+] Gathering DNS records for $subdomain ..."
